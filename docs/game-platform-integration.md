...
- **Score Polling**: Hourly Vercel Cron job hits `/api/game-platform/sync/scores?secret=***` (secret stored as `GAME_PLATFORM_CRON_SECRET`) to hydrate `game_platform_stats`; this same endpoint can be triggered on-demand with `dryRun=true` via POST.
