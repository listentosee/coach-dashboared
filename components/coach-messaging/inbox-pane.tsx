  ...
  const threadEntries = useMemo(() => {
    ...
    for (const conversation of filteredConversations) {
      ...
      for (const group of groups) {
        ...
        entries.push({ conversation, group })
      }
    }
        ...
        {listMode === 'threads' ? (
          threadEntries.length === 0 ? (
            ...
            <div className="space-y-2 px-4 py-3">
              {threadEntries.map(({ conversation, group }) => {
                ...
                const threadSubject = group.subject || plainTextSnippet(rootMessage.body) || conversationTitle
          ...
          <div className="space-y-1 px-4 py-3">
            {messageEntries.map(({ conversation, message, group }) => {
              ...
              const threadSubject = group?.subject || plainTextSnippet(threadMessages[0]?.body || '') || resolveConversationTitle(conversation)
