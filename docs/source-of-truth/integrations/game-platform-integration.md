...
## 23. Work Based Learning Hours Report (2026-07-03)

Coach-facing report under Coach Tools (`/dashboard/work-based-learning-hours`) that estimates each of a coach's students' platform engagement time. Reviewable online (mirrors the workbook Detail) and exportable to Excel; scoped strictly per coach.

- **RPC:** `get_work_based_learning_hours(p_synced_user_ids text[], p_start timestamptz, p_end timestamptz, p_gap_minutes int=30, p_tail_minutes int=10, p_orphan_minutes int=15, p_ctf_regular_minutes int=120, p_ctf_mayors_minutes int=210, p_mayors_name text='Inland Empire Mayors Cyber Cup 2026')` → `(synced_user_id, segment, activity, solves, sessions, minutes)`. `LANGUAGE sql STABLE`, GRANTed to `authenticated, service_role`. Migration `20260703000000_work_based_learning_hours_rpc.sql` (applied to prod 2026-07-03).
- **Estimation model.** On-Demand time = sessionize `source='odl'` solves per canonical challenge type: a gap > 30 min starts a new session; a session of ≥2 solves = (last − first) + 10 min; a lone solve (orphan) = 15 min. Flash CTF time = per event with `challenges_solved > 0`, credit the FULL event window (Inland Empire Mayors Cyber Cup = 3.5 h; every other/regular monthly Flash CTF = 2 h). The 2 h / 3.5 h windows are **rule-based** because `game_platform_flash_ctf_events.ended_at` is not recorded. All assumptions are RPC parameters (defaults above).
- **Invariants.** Sessionize on `solved_at`, never `created_at` (which is sync-ingest time). Read the solves/events tables via the service-role stats client + this RPC (RLS + PostgREST 1000-row cap) — never a broad `.select()`. **Coach scoping is per `coach_id` only; there is no county/`school_geo` logic anywhere in the feature.** `flash_ctf_events.challenges_solved` reconciles 1:1 with `source='flash_ctf'` solve counts.
- **Surfaces.** Pure helpers `lib/integrations/game-platform/work-based-learning-hours.ts` (types, `groupWblRows`, `summarizeWbl`, `fetchWblRows`, `divisionLabel`) + period presets `lib/reports/wbl-periods.ts` (default 2025–26 school year; Fall/Spring/All-time) + shared loader `lib/reports/work-based-learning-hours.ts` (`loadWblReport`) + ExcelJS builder `lib/reports/wbl-workbook.ts` (4 sheets incl. a required **Methodology** sheet for submission legitimacy). Routes: `GET /api/coach-reports/work-based-learning-hours` (JSON) and `…/export` (xlsx). Admin act-as via the `admin_coach_id` cookie, same as the dashboard. Verified live all-time total ≈ 2,345.1 h for the Riverside + San Bernardino population (On-Demand 1,338.1 h + Flash CTF 1,007.0 h).
