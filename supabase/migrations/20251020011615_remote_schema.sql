...
CREATE OR REPLACE VIEW "public"."release_eligible_competitors" WITH ("security_invoker"='on') AS
 SELECT "id",
  ...
  WHERE ("is_active" AND ("status" = ANY (ARRAY['profile'::"text", 'compliance'::"text", 'complete'::"text"])));
...
CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION handle_new_user();
  ...
  create policy "Authenticated User Access jrj208_0"
  on "storage"."objects"
  as permissive
  for select
  to authenticated
using ((bucket_id = 'team-images'::text));
  ...
  create policy "Authenticated User Access jrj208_1"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
with check ((bucket_id = 'team-images'::text));



  create policy "Authenticated User Access jrj208_2"
  on "storage"."objects"
  as permissive
  for update
  to authenticated
using ((bucket_id = 'team-images'::text));
  ...
  create policy "Authenticated User Access jrj208_3"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
using ((bucket_id = 'team-images'::text));
  ...
  create policy "message_images 1rdzryk_0"
  on "storage"."objects"
  as permissive
  for select
  to authenticated
using ((bucket_id = 'messages'::text));
  ...
  create policy "message_images 1rdzryk_1"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
with check ((bucket_id = 'messages'::text));
  ...
  create policy "message_images 1rdzryk_2"
  on "storage"."objects"
  as permissive
  for update
  to authenticated
using ((bucket_id = 'messages'::text));
  ...
  create policy "message_images 1rdzryk_3"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
