-- Supabase migration: Messaging enhancements (read receipts, threading, 'group' type)

begin;

-- Extend conversation types and policy
...
-- Read receipts table and policies
...
-- Threading columns, indexes, and triggers
...
returns table (conversation_id uuid, unread_count integer, last_message_at timestamptz)
language sql stable security definer set search_path = public as $$
