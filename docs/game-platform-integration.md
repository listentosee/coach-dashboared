...
- **Score Polling**: Hourly Vercel Cron job hits `/api/internal/sync` using a secret header (`x-internal-sync-secret`). The route uses a Supabase service-role client to sync stats; admins can trigger the same route via POST (`/api/internal/sync`, body `{ dryRun?: boolean, coachId?: string }`).
